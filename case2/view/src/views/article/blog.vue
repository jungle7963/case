  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="分类" width="110px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.cate_id | cateIdFilter(catelist) }}</span>
        </template>
      </el-table-column>
...
import { getListcate } from '@/api/categery'
...
export default {
  ...
  filters: {
    ...
    cateIdFilter(cateId,catelist){
      for (let i=0;i< catelist.length;i++)
      {
        if (cateId == catelist[i]['id']){
          cateId = catelist[i]['name']
        }
      }
      return cateId;
    }
